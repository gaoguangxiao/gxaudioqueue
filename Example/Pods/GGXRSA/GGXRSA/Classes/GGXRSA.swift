...
//  TDSwiftTemplate
//
//  Created by 高广校 on 2024/07/19.
//

import Foundation

public struct GGXRSA {
    ...
    private static func loadPublicKey(_ path: String) -> SecKey? {
        ...
        guard let trust, trustCreationStatus == errSecSuccess else { return nil }
...
//        var key: SecKey?
        ...
        // p12 文件可以包含多个证书和私钥
        guard let transformItems = items as? Array<Dictionary<String, Any>>
            else {
            return nil
        }
        let p12Attrs = transformItems[0]
        //提取`Identity`对象
        let identity: SecIdentity = p12Attrs[kSecImportItemIdentity as String] as! SecIdentity
        //从`Identity`对象提取私钥
        var privateKeyRef: SecKey?
        let privateKeystatus = SecIdentityCopyPrivateKey(identity, &privateKeyRef)
