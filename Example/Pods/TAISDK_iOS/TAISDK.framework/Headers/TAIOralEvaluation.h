...
// 异步模式标识
//0：同步模式（默认）
//1：异步模式（一般情况不建议使用异步模式，如需使用参考：异步轮询）
//可选值参考服务模式。
@property (nonatomic, assign) BOOL isAsync;

/**
 * 服务器返回评测超时，客户端重试次数, 默认5次
 */
@property (nonatomic, assign) NSInteger serviceTimeoutTryTimes;

/**
 * 服务器返回评测超时，客户端重试时间间隔, 单位ms， 默认300ms
 */

@property (nonatomic, assign) NSInteger serviceTimeoutWaitInterval;

/**
 * 音频存储路径，支持通过子路径指定文件夹名称
 示例值：https://test-appid.cos.ap-guangzhou.myqcloud.com/folder1/folder2
 */
@property (nonatomic, strong) NSString *cosBucketURL;
...
/**
 * 口语评测（外部录制）,适用于较大的音频文件
 * @param param 参数
 * @param path 音频文件路径（三种格式目前仅支持16k采样率16bit编码单声道，如有不一致可能导致评估不准确或失败）
 * @param callback 回调
 */
- (void)oralEvaluation:(TAIOralEvaluationParam *)param path:(NSString *)path callback:(TAIOralEvaluationCallback)callback;
